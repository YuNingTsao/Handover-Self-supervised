...
class upsample(nn.Module):
    def __init__(self, in_channels, out_channels, data_shape,
        ...
        self.siamses_last_conv = nn.Sequential(nn.Conv2d(304, 256, kernel_size=3, stride=1, padding=1, bias=False),
    ...
    def forward(self, x, data_shape=None):
        siamses_f = self.siamses_last_conv(x)
        f = self.last_conv(siamses_f)
        ...
        return siamses_f, F.interpolate(pred, size=(h, w), mode='bilinear', align_corners=True)
...
class DecoderNetwork(nn.Module):
    ...
    def forward(self, f, data_shape=None):
        siamses_pred, pred = self.upsample(f, data_shape)
        return siamses_pred, pred
...
class VATDecoderNetwork(nn.Module):
    ...
    def forward(self, fa=None, icg=None, data_shape=None, t_model=None):
        if t_model is not None:
            r_adv = get_r_adv_t(fa, icg, t_model[0], t_model[1], it=1, xi=1e-6, eps=2.0)
            fa += r_adv

        siamses_pred, pred = self.upsample(fa, data_shape)
        return pred
